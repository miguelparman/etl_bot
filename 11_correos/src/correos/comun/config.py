...
172.17.0.162), destino de bronze/silver/gold."""
...
from comun.exceptions import ConfiguracionError
...
@dataclass(frozen=True)
class DbSettings:
    """SQL Server 'CL_MOVIL' (172.17.0.162), destino de bronze/silver/gold.
    Separado de Settings/cargar_configuracion() (igual que
    SharePointOrigenSettings/cargar_configuracion_origen() en 02_ventas):
    la ingesta (copia/verificacion) no necesita estas variables."""
...
class Settings:
    ...
    # Periodo de carga de bronze/silver/gold (FechaHora_UTC_Texto, UTC).
    # Sin parsear/validar aqui -- eso lo hace comun/periodo.resolver_periodo().
    # No usado por la ingesta.
...
def cargar_configuracion(
    ...
    (ver main.py --fecha-inicio/--fecha-fin) el periodo definido en
...
def cargar_configuracion_db(base_dir: Path) -> DbSettings:
    """Carga las credenciales de SQL Server ('CL_MOVIL'), usadas por las etapas
    bronze/silver/gold."""
    load_dotenv(base_dir / ".env")
    return DbSettings(
