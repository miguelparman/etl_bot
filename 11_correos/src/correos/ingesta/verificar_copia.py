"""INGESTA: audita, despues de la copia, que haya preservado el FORMATO de
cada Excel: mismas hojas 'Registro' y 'Bandejas', mismas columnas
...
Uso:
    python main.py --verificar-copia
"""
...
def ejecutar_verificacion(settings: Settings) -> bool:
    """Verifica todos los archivos y registra el detalle en el log. Devuelve
    True si todos coinciden."""
    cliente_destino, drive_id_destino = conectar_destino(settings.destino)
    resultados = verificar_copia(settings.origen, cliente_destino, drive_id_destino, settings.destino.folder_path)
