...
- Silver: TBL_CORREO_REGISTRO_SILVER -- se construye LEYENDO bronze (no los
  ...
  (mappings.COLUMNAS_EXCLUIDAS_SILVER), y agregando columnas derivadas. Hoy:
  ASUNTO_AGRUPADO (ver mappings.REGLAS_ASUNTO_AGRUPADO).
- Gold: modelo estrella en el esquema 'gold' -- ver gold.py.
...
def _select_bronze(con_periodo: bool) -> str:
    ...
    sql = f"SELECT {columnas_sql} FROM [{ESQUEMA}].[{TABLA_REGISTRO}]"
    if con_periodo:
...
def recargar_silver_completo(gateway) -> int:
    ...
    logger.info("%s fila(s) leidas de [%s].[%s] (bronze, completo).", len(bronze_df), ESQUEMA, TABLA_REGISTRO)
    silver_df = construir_silver(bronze_df)
    _log_reparto(silver_df)
