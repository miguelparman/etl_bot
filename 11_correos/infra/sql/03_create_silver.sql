-- Capa SILVER de la estructura medallion (ver src/correos/silver.py):
-- mismas columnas que la capa bronze TBL_CORREO_REGISTRO (ver
-- 02_create_tables.sql) + columnas derivadas. La llena cargar_silver.py /
-- main.py leyendo bronze, nunca los .xlsx directamente.
--
...
BEGIN
    CREATE TABLE dbo.TBL_CORREO_REGISTRO_SILVER (
        ...
        ASUNTO_AGRUPADO                   NVARCHAR(50)   NULL   -- REBOTE / PRESENTACIÓN / PRUEBA / PROMO; NULL si ninguna regla calza (ver mappings.REGLAS_ASUNTO_AGRUPADO)
    );
...
-- (ver mappings.COLUMNAS_EXCLUIDAS_SILVER).
