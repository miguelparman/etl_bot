...
-- reales de los 'Registro_*.xlsx' consolidados por bronze (ver
-- src/correos/bronze/consolidar.py y mappings.py) -- no en INFORMATION_SCHEMA contra el servidor
...
-- main.py. Si algun tipo real difiere (ej. un NVARCHAR mas corto,
...
BEGIN
    CREATE TABLE dbo.TBL_CORREO_REGISTRO (
        ...
        FechaHora_UTC_Texto               DATETIME2(7)   NULL,  -- convertido desde texto ISO-8601 UTC; campo de control de periodo (ver bronze/cargar.py)
    ...
    -- Soporta el DELETE/INSERT por rango de FechaHora_UTC_Texto (ver bronze/cargar.cargar()).
