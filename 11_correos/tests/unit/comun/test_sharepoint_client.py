...
from comun.sharepoint_client import SharePointClient
...
def test_ignora_subcarpetas_y_archivos_no_excel(monkeypatch):
    ...
    monkeypatch.setattr("comun.sharepoint_client.requests.get", lambda *a, **kw: _FakeResponse(payload))
...
def test_sigue_la_paginacion(monkeypatch):
    ...
    monkeypatch.setattr("comun.sharepoint_client.requests.get", lambda *a, **kw: _FakeResponse(next(respuestas)))
