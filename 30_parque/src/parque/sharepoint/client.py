...
resolve_site/resolve_drive/resolve_folder/resolve_default_drive son el mismo
patron ya usado en 01_servidor_chile/sharepoint/client.py y
201_bot_salesforce/app/sharepoint/client.py (que solo SUBEN archivos).
Ningun proyecto del repositorio descarga contenido todavia -- download_file
es nuevo, agregado aqui siguiendo la misma forma de cliente Graph, porque
30_parque necesita LEER los CSV publicados en SharePoint (reemplazo del
Connection Manager OLE DB 'srv_chile' del .dtsx original).
"""
...
class SharePointClient:
    ...
    def download_file(self, drive_id: str, file_path: str) -> bytes:
        ...
        dentro del drive (carpeta + nombre de archivo). Equivalente de
        lectura al 'PUT .../content' que usa SharePointUploader.upload en
        01_servidor_chile/sharepoint/uploader.py (que solo sube); 'requests'
        sigue automaticamente la redireccion que Graph hace hacia la URL de
