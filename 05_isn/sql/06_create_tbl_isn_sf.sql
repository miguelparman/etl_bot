-- Origen SSIS: Execute SQL Task "TBL_ISN_SF" (dentro de "SF")
-- Conexion original: 162.CL_ISN
-- Parametros originales (SSIS ParameterBinding "0"/"1" -> User::Fecha_inicio / User::Fecha_fin),
-- aqui expresados como parametros nombrados de SQLAlchemy: :fecha_inicio, :fecha_fin
--
-- LOGICA DE NEGOCIO CRITICA: 18 LEFT JOIN entre CL_VISTAS/CL_ANALISIS/CL_CARTERA/CL_CALIDAD/
-- CL_ISN/BBDD_GENERAL. Se preserva TAL CUAL (incluyendo comentarios `--` del autor original)
-- porque documentan reglas de negocio historicas. NO modificar sin validar con negocio.
SELECT
